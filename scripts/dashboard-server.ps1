...
function Get-DashboardHtml {
    ...
    foreach ($a in ($projectAgents + $globalAgents)) {
        ...
        $shortDesc = $a.Description -replace ' --.*$', '' -replace ' —.*$', ''
...
"@
    ...
    if (Test-Path "$projectRoot\learnings.md") {
        ...
        $learningBlocks = [regex]::Matches($learningsContent, '### (\d{4}-\d{2}-\d{2}) . (.+)\r?\n(.+)')
        $count = 0
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .refresh-hint { color: #555; font-size: 12px; margin-left: 16px; }
    .grid {
      display: grid;
...
<body>
  ...
  <h1>Dev-Agentur Dashboard <span class="refresh-hint">F5 = aktualisieren</span></h1>
...
$learningsHtml
  ...
  <p class="footer">Dev-Agentur von Claas &mdash; localhost:$port</p>
...
try {
    $listener.Start()
    Write-Host "Dashboard laeuft auf http://localhost:$port/" -ForegroundColor Green
